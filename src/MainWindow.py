...
class MainWindow:
    ...
    def defineComponents(self):
        ...
        self.menu_aboutmauna = UI("menu_aboutmauna")
        ...
        self.dialog_about.set_program_name(_("Mauna My Computer"))
        # Set version
    ...
    def set_os_label_img(self):
        ...
        if os_id == "mauna":
            ...
            self.img_os_menu.set_from_icon_name("emblem-debian-symbolic", Gtk.IconSize.BUTTON)
            self.lbl_os_menu.set_label("{}".format(_("About Mauna")))
        else:
            try:
    ...
    def get_controlpanel_desktops(self):
        blacklist = ["synaptic.desktop", "top.mauna.mycomputer.desktop"]
        apps = []
            ...
            if file_info is not None:
                ...
                try:
                    if file_info["usage_percent"] >= 0.9:
                        row_volume._pb_volume_size.get_style_context().add_class("mauna-mycomputer-progress-90")
                except Exception as e:
    ...
    def addDisksToGUI(self):
        ...
        try:
            if root_info["usage_percent"] >= 0.9:
                self.pb_root_usage.get_style_context().add_class("mauna-mycomputer-progress-90")
        except Exception as e:
            print("progress css exception: {}".format(e))
    ...
    def on_btn_format_removable_clicked(self, button):
        ...
        subprocess.Popen(["mauna-usb-formatter", file_info["device"]])
    ...
    def on_menu_aboutmauna_clicked(self, button):
        self.popover_menu.popdown()

        desktop = self.get_current_desktop()

        try:
            subprocess.Popen(["mauna-about"])
        except Exception as e:
            print("{}".format(e))
            if "xfce" in desktop:
                try:
